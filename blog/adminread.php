...
<body>
    ...
    ?>
    <?php
    if (!empty($_GET['urlDelete'])) {
        $del = mysqli_query($db, "DELETE FROM blogpost WHERE URL = '$_GET[urlDelete]'");
        if ($del) {
            echo "<script>
                    alert('Deleted');
                ...
                <section class="section-3">
                    <div class="container">
                        ...
                        if (!empty($_GET['News']) or !empty($_GET['Article']) or !empty($_GET['Event'])) {
                            $fetchNews = mysqli_query($db, "SELECT * FROM blogpost WHERE URL = '$_GET[News]' or URL = '$_GET[Article]' or URL = '$_GET[Event]' ORDER BY id DESC");
                            ...
                            <p></p>";
                                ...
                                echo "<a href ='adminedit.php?$post[TYPE]=$post[URL]' name='edit' class='button' style='background-color:#4CAF50; margin: 10px 0;  border-color:#4CAF50;float:right; font-size:18px;' type='button'>Edit</a>";
                                echo "<a href ='adminread.php?urlDelete=$post[URL]' class='button' style='background-color:#4CAF50; margin: 10px 0;  border-color:#4CAF50;float:right; font-size:18px;margin-left:0.5em' type='button'>Delete</a>";
                            } else {
